...
async function trackFullTimeline() {
    ...
    for (let i = 1; i <= 230; i++) {
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                // Extremely permissive yellow to catch all Motion Blur leading/trailing edges
                // Normal yellow is ~ R:255 G:210 B:30
                // Background is deep blue ~ R:0 G:45 B:180
                if (r > 60 && g > 50 && b < 140 && (r > b)) {
                    if (x < minX) minX = x;
                    if (x > maxX) maxX = x;
                    if (y < minY) minY = y;
                    if (y > maxY) maxY = y;
    ...
    fs.writeFileSync('box_tracking_mblur.json', JSON.stringify(positions, null, 2));
    console.log("Motion blur tracking saved!");
}
